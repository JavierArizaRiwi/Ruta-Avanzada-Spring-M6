# Día 2 - Validaciones Avanzadas y Manejo Global de Excepciones en Spring Boot (Arquitectura Hexagonal)
...
## 5) ErrorResponse uniforme (contrato)
...
3. **Ejecutar pruebas:** `mvn -q -Dtest=*Validation* test` o desde el IDE.  
4. **Probar con curl:**  
   ```bash
...
- Respuestas uniformes de error con `timestamp`, `status`, `error`, `message`, `path`, `fields`.  
